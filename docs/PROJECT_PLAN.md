...
> 状态：P0–P5 已完成；P6 功能回归有证据，但产品/视觉未通过用户验收。当前恢复入口见 `docs/project-context/README.md`
> 日期：2026-07-09
...
3. **证据与解释**
   ...
   - tensor 详情面板。
   - 源码与旧教程的证据链接。
   - 完整 English / 中文界面与术语切换；English 为默认语言。
...
采用长页面 scrollytelling 与全宽自然文档流，顶部控制保持固定。视觉语言参考 Transformer Explainer 的“矩阵块 + 连续流程线 + 点击展开 + 教科书解释”，但不复制其 GPT-2 结构或页面布局。页面不是 1440×1024 的单屏仪表盘，也不是静态图片轮播。

页面包含：

- 顶部 sticky 控制：prompt、播放/暂停、单步、速度、阶段和 `EN / 中文`。
- 纵向章节：初始化、Token、Prefill、Attention、MoE/W8A8、TP、Logits/Decode、Evidence。
- 全宽主画布：滚动或播放驱动同一份连续状态。
- 点击展开：节点在同一画布内平滑进入内部计算，再返回原播放位置。
- 全局时间线：从初始化到第 5 个 decode token。
- Evidence drawer：解释、tensor 检查、真实性、源码和 run provenance，默认折叠。
- 故事模式与探索模式共享同一状态，不复制页面。
...
- 完成信息架构、播放状态机和模块接口。
- 提供 3 个视觉方向；根据用户反馈融合为“全局长卷 + 矩阵剧场 + TP 双轨章节”。
- 冻结长页面、点击连续展开、English 默认和中英文切换。
- 产出融合视觉稿；验收后再搭建页面。
...
当前仍待后续确认：项目是否公开、是否允许发布小规模权重/激活采样，以及是否长期保留当前 prompt。视觉方向已确认为故事主路径与工程探索层共存。
